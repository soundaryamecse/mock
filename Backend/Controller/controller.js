...
const getTeacherData = (req,res) =>{
    ...
    if(query){
        // const search =new RegExp(`${query}+`,'g')
        const search = new RegExp(modifyTextToRegex(query),'gi')
        Teacher.find({name : search}).then(teachers => res.json(teachers)).catch(err=>res.status(400).json("Error:"+ err))    
    }
...
const getTeachersPage= async (req, res) => {
    ...
    try {
        if(query){
            const search = new RegExp(modifyTextToRegex(query),'gi')
            // const search =new RegExp(`${query}+`,'g')
            results.current = await(Teacher.find({name:search}).limit(limit).skip(startIndex).exec())
            res.json(results)
        }
        if(filter && age==="" && sort===""){
